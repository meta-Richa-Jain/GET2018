...
public class EmployeeDAOImpl {
	...
	public boolean updateProfile(Employee employee) {
		return jdbcTemplate.update(UPDATE_PROFILE, employee.getDob(),
				employee.getPrimaryContactNo(),
				employee.getSecondaryContactNo(), employee.getSkypeId(),
				employee.getProfilePicture(), employee.getCode()) > 0;
	}

	public Login checkLoginDetails(int code) {
		return jdbcTemplate.queryForObject(LOGIN_DETAILS, new LoginMapper());
	}
