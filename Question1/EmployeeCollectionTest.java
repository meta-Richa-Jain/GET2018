...
public class EmployeeCollectionTest {
	...
	public void testSortById() throws InvalidInputError {
		...
		for(int count=0;count<expectedResult.size();count++){
			assertEquals(expectedResult.get(count).getEmpId(), employeeCollection.employeeList.get(count).getEmpId());
		}
	...
	public void testSortByName() throws InvalidInputError {
		...
		for(int count=0;count<expectedResult.size();count++){
			assertEquals(expectedResult.get(count).getName(), employeeCollection.employeeList.get(count).getName());
		}
	...
	public void testDuplicateId() throws InvalidInputError {
		assertEquals(true,employeeCollection.addEmployee(1, "arjita","gwalior"));
		...
		assertEquals(false,employeeCollection.addEmployee(5, "aastha", "bikaner"));
	}
